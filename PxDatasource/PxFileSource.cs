...
namespace PxDatasource
{
    public class PxFileSource : IDatasource
    {
        ...
        public IPXModelBuilder GetBuilder(string language, string path)
        {
            ...
            builder.SetPath(System.IO.Path.Combine($@"wwwroot\db\{ DatabaseId}\", path));
            return builder;
        }

        public Item GetMenu(string db, string language, string path)
        {
            string menuXMLPath = $@"wwwroot\db\{DatabaseId}\menu.xml";
            ...
            ItemSelection cid = string.IsNullOrEmpty(path) ? new ItemSelection() : new ItemSelection(System.IO.Path.GetDirectoryName(db + "\\" + path), System.IO.Path.GetFileName(path));
            ...
            if (cid.Menu != "START" && cid.Selection != "START")
            {
                ...
                else if (((path.ToLower().Contains(".px")) && (!path.Contains(@"\"))) &&
                          ((menu.CurrentItem.ID.Menu.Contains(@"\") && menu.CurrentItem.ID.Selection == path)))
                {
